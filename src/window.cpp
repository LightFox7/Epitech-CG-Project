...
bool Window::InitWindow(const std::string_view& name, uint16_t width, uint16_t height, cxxopts::ParseResult& commandLineOptions)
{
    if (!m_IsInitialized)
    {
...

        {
...

            {
...

                {
                    ...
                    const int32_t contextFlags = useDebugContext ? WGL_CONTEXT_DEBUG_BIT_ARB : GL_NONE;
                    ...
                    // Only add this context attribute if we have the extension
                    const int32_t noErrorAttribute = WGLEW_ARB_create_context_no_error ? WGL_CONTEXT_OPENGL_NO_ERROR_ARB : 0;
                    const int32_t noErrorFlag = WGLEW_ARB_create_context_no_error && !useDebugContext ? GL_TRUE : GL_FALSE;
        ...
        // Removes potential errors that might have been put in the stack at context creation
